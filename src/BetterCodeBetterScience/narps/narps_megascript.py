    ...
    for type in ['thresh', 'unthresh']:
        for img_file in team_orig_dir.glob(f"*_{type}.nii.gz"):
            ...
            try:
                ...
                print(f"Unexpected hypothesis number format in file {img_file}, skipping.")
                continue
            dest_file = datadir / f"team-{team_id_short}_hyp-{hyp}_type-{imgtype}_space-native_desc-orig_stat.nii.gz"
...
for unthresh_img_path in unthresh_images:
    ...
    if not Path(thresh_img_path).exists():
        print(f"Thresholded image not found for hyp {hyp_num}, team {team_id}, moving to excluded data.")
        shutil.move(unthresh_img_path, datadir_excluded / unthresh_img_path.name)
    ...
    if unthresh_img.shape != thresh_img.shape or not np.allclose(unthresh_img.affine, thresh_img.affine):
        print(f"Image shape or affine mismatch for hyp {components['hyp']}, team {components['team']}, moving to excluded data.")
        shutil.move(unthresh_img_path, datadir_excluded / unthresh_img_path.name)
        shutil.move(thresh_img_path, datadir_excluded / Path(thresh_img_path).name)
    ...
    if n_thresh_vox > 0:
        ...
        if min_p_direction > error_thresh:
            ...
            print(f"hyp {hyp_num}, team {team_id} - invalid in-mask values (neg: {p_neg_unthresh:.3f}, pos: {p_pos_unthresh:.3f})")
...
print (f"Found {len(thresh_images_to_binarize)} thresh binarized images to process.")

results = {}
overwrite = False
thresh = 1e-4
for thresh_img_path in thresh_images_to_binarize:
    outfile = Path(modify_bids_filename(
        thresh_img_path,
        desc='binarized', suffix='mask'
    ))
...
mni_template = tflow.get("MNI152NLin2009cAsym", resolution=2, suffix="T1w", desc=None)

print("Resampling images to MNI space...")
results = {}
overwrite = False
all_images_to_resample = find_bids_files(
    datadir, type='thresh', space='native', desc='binarized') + find_bids_files(
    datadir, type='unthresh', space='native', desc='rectified') 
...
for img_path in all_images_to_resample:
    ...
    resampled_img = resample_to_img(img, mni_template, 
...
for hyp in range(1,10):
    ...
    resampled_images = {'unthresh': find_bids_files(
    ...
    for img_path in resampled_images['unthresh']:
        ...
        thresh_img_path = modify_bids_filename(
            img_path,
            type='thresh', desc='binarized', suffix='mask'
        )
        assert Path(thresh_img_path).exists(), f"Binarized thresholded image not found for {img_path}"
        resampled_images['thresh'].append(thresh_img_path)
    assert len(resampled_images['thresh']) == len(resampled_images['unthresh']), "Mismatch in number of unthresh and thresh images"
    print (f"Found {len(team_ids)} unthresh resampled images to concatenate for hypothesis {hyp}.")
    ...
    for imgtype in ['unthresh', 'thresh']:
        ...
        output_path = concat_dir / f"hyp-{hyp}_type-{imgtype}_space-MNI152NLin2009cAsym_desc-concat_{suffix_dict[imgtype]}.nii.gz"
        concat_img.to_filename(str(output_path))
        print(f"Saved concatenated {imgtype} image for hypothesis {hyp} to {output_path}")
        results[str(output_path)] = {
            'team_ids': team_ids
        }
